const fs = require("fs");
const re_normal = /(\w+\s+\w+\s+=\s+require\(.+?\)\;)$/gm; // something like const name = require("name");
const re_unique = /(\w+\s+\w+\s+=\s+require\(.+?\).\w+.+)$/gm; //something like const Bob = require("name").first
