  ...
  fs.readFileSync(file)
    ...
    .forEach(line => {
      if (line.match(re_normal)) {
        fs.appendFileSync(
          file + "_bak",
          line.replace(re_normal, `import $2 from $3`) + "\n"
        );
      } else if (line.match(re_unique)) {
        fs.appendFileSync(
          file + "_bak",
          line.replace(re_unique, `import { $4 as $2 } from $3`) + "\n"
        );
      } else if (line.match(re_special)) {
        fs.appendFileSync(
          file + "_bak",
          line.replace(re_special, `import $3 from $4`) + "\n"
        );
      } else {
